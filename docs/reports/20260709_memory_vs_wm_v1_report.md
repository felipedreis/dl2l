...
3. Sleep-based adapter consolidation adds benefit or cost on top of the JEPA filter.
...
| # | Hypothesis |
|---|-----------|
| H1 | JEPA increases creature survival vs. baseline in both raw and corrected seconds |
...
| JEPA+Consol | 543.59 | 216.73 | 25 |
| **JEPA** | **720.37** | 342.64 | 25 |
...
| Baseline vs JEPA+Consol | < 0.0001 | *** |
| Baseline vs JEPA | 0.0002 | *** |
| JEPA+Consol vs JEPA | 0.0991 | ns |

Both JEPA conditions survive significantly longer than baseline in raw seconds. JEPA
(720s) numerically outperforms JEPA+Consol (544s), but the difference is not significant
...
|-----------|:----------:|:----:|:-:|
| **JEPA** | **22,867** | 10,467 | 25 |
| Baseline | 21,417 | 2,307 | 25 |
| Mem+Consol | 19,844 | 3,059 | 25 |
| Memory | 19,270 | 3,960 | 25 |
| JEPA+Consol | 19,122 | 8,104 | 25 |

Kruskal-Wallis: H = 11.346, p = 0.023.

JEPA (no consolidation) achieves the highest tick count (22,867 — numerically above
baseline at 21,417, though p = 0.352 ns). JEPA+Consol is significantly below baseline
...
| JEPA+Consol | 19,122 | 543.6 | 35.2 | 228.2 | 315.4 |
| **JEPA** | **22,867** | 720.4 | 31.7 | 279.1 | **441.2** |
...
- JEPA: 441s — well above baseline (290s), p = 0.0008 \*\*\*
- JEPA+Consol: 315s — comparable to baseline (p = 0.352 ns)

> **H1: Confirmed for JEPA; not for JEPA+Consol.** JEPA without consolidation
...
> JEPA+Consol corrected lifetime (315s) is not distinguishable from baseline (p = 0.35 ns),
> and its tick count is significantly below baseline (p = 0.003).

> **H2: Not confirmed — consolidation hurts.** Removing consolidation from JEPA
...
| JEPA+Consol | 15.48 | 3.38 |
| **JEPA** | **15.20** | 3.49 |

Both JEPA conditions show substantially lower mean arousal (~15.2–15.5 vs ~17.2–17.3 for
non-JEPA), with JEPA achieving the lowest value overall. The arousal suppression is
...
| Drive | Baseline | Memory | Mem+Consol | JEPA+Consol | JEPA |
|-------|:--------:|:------:|:----------:|:---------------:|:--------:|
...
Both JEPA conditions produce dramatically lower Tedium (0.74–0.82 vs 2.33–2.43 baseline —
a 66–70% reduction). JEPA (no consolidation) achieves the lowest Tedium of all conditions
...
| JEPA+Consol | 48.9% | 23.3% | — | **25.0%** | 2.6% |
| JEPA | 49.1% | 22.5% | — | **25.4%** | 2.9% |

Both JEPA conditions fire WORLD_MODEL at ~25% of cycles. JEPA (no consolidation) shows
...
| JEPA+Consol | 565 (27%) | 940 (45%) | 590 (28%) | 2,095 |
| JEPA | 887 (31%) | 1,089 (38%) | 908 (31%) | 2,884 |

JEPA+Consol shows the best food quality selectivity — fewer Gray Apple (27%) and more
Green Apple (45%) than baseline (30% gray, 40% green). JEPA without consolidation eats
...
| JEPA+Consol | 5.328 | 1.504 | 2,095 |
| JEPA | 5.331 | 1.488 | 2,884 |
...
| JEPA+Consol | 28,008 | 56.3% |
| JEPA | 33,425 | 54.6% |

Memory conditions retain the best cactus avoidance (57–58%). JEPA+Consol recovers to
56.3%, approaching Memory-condition levels. JEPA without consolidation drops to 54.6%
...
Cumulative food-type curves show JEPA+Consol maintaining consistently higher Green Apple
selection across the full lifetime (37.6% Green per decile vs 30.7% for baseline). JEPA
...
Tedium suppression in JEPA (Section 5) points to a more active novelty/curiosity signal
...
| **JEPA+Consol** | **0.6625** | 2.582 |
| **JEPA** | **0.6516** | 2.474 |

Both JEPA conditions generate mean |RPE| of ~0.65 — approximately **15× larger** than
...
> **H5: Confirmed.** Both JEPA conditions produce RPE signals 15× larger than the tabular
...
| JEPA+Consol | 237,238 | 0.215 | **0.1445** |
| **JEPA** | **292,898** | 0.216 | **0.1433** |

Both JEPA conditions show `mean|emotion_delta|` ~0.143–0.145 — **14× larger** than
...
JEPA (no consolidation) has the highest absolute engram count (292,898 — 23% more than
JEPA+Consol's 237,238), consistent with its higher tick count producing more memory traces.
...
| JEPA+Consol | 329 | 11.12 | 1.88 |
| **JEPA** | **415** | **11.06** | 1.97 |

JEPA (no consolidation) has the most sleep episodes by far (415 — 31% more than
JEPA+Consol's 329), consistent with its longer raw lifetime (720s). Both JEPA conditions
...
|-----------|------:|:---------:|:-----------:|:--------:|
| JEPA+Consol | 119,677 | 47.7 | 44 | 338 |
| JEPA | 145,008 | 48.1 | 45 | 251 |
...
JEPA (145k vs 120k) directly reflects its higher tick count and longer lifetime.
...
- **JEPA (cond 4, no consol):** 720s raw / 441s corrected — the strongest survival
  result in this experiment; 52% above baseline in corrected terms (p = 0.0008 \*\*\*).
- **JEPA+Consol (cond 5):** 544s raw / 315s corrected — above baseline in raw seconds
...
The central finding is that **removing consolidation from JEPA improves performance**:
...
Both JEPA conditions show dramatically lower Tedium (0.74–0.82 vs 2.33–2.43 for
non-JEPA — a 66–70% reduction). JEPA without consolidation achieves the lowest Tedium
...
JEPA+Consol (cond 5) avoids cacti at 56.3% — better than JEPA (cond 4, 54.6%)
...
| Metric | Baseline | Memory | Mem+Consol | JEPA+Consol | **JEPA** |
|--------|:-------:|:------:|:----------:|:---------------:|:------------:|
...
**H1: Confirmed for JEPA (no consolidation).** JEPA (cond 4) survives 441s
corrected vs 290s baseline (p = 0.0008 \*\*\*) and achieves the highest tick count (22,867 vs
21,417 baseline, p = 0.35 ns trend). JEPA+Consol (cond 5) does not show a significant
corrected advantage over baseline (315s vs 290s, p = 0.35 ns), as consolidation overhead
erases the benefit.

**H2: Not confirmed — consolidation hurts in a familiar world.** JEPA without
consolidation (cond 4) outperforms JEPA with consolidation (cond 5) by 40% in corrected
...
**H5: Confirmed.** Both JEPA conditions generate |RPE| ≈ 0.65 — 15× larger than
...
1. **JEPA is the winning condition for familiar worlds.** Remove consolidation
...
3. **Extended JEPA run.** The Tedium suppression (0.74 vs 2.43 baseline) and high tick
   count suggest JEPA creatures may show interesting long-horizon behaviour. Run with
   `maxRuntimeMinutes=240` to observe whether the effect is stable over longer simulations.

4. **Disentangle food quality preference from spatial effects.** JEPA+Consol shows
   better Green Apple selection (45%) than JEPA (38%). Run a controlled experiment with
