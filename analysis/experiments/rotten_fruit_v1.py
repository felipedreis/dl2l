...
This is the dl2l_analysis-based port of analysis/exp_rotten_fruit_v1.py — same
figures, same stats, same numbers; the shared loading/stats/figure scaffold
now lives in analysis/dl2l_analysis/ instead of being duplicated here.

Usage:
  python3 -m dl2l_analysis --experiment rotten_fruit_v1
  (or directly: python3 -m analysis.experiments.rotten_fruit_v1)
"""
...
from analysis.dl2l_analysis.config import Condition, ExperimentAnalysis
...
def default_config() -> ExperimentAnalysis:
    """Standalone config matching the CONDITIONS/PALETTE hardcoded in the
    original analysis/exp_rotten_fruit_v1.py. Once experiments/rotten_fruit_v1.yml
    exists (Phase 3), replace call sites with
    ExperimentAnalysis.from_spec("rotten_fruit_v1")."""
    conditions = [
        Condition("1_baseline", "Baseline", "#9e9e9e"),
        Condition("2_memory_only", "Memory", "#5c85d6"),
        Condition("3_memory_consolidation", "Mem+Consol", "#2b5eb8"),
        Condition("4_jepa_rpe_only", "JEPA", "#b05ec4"),
        Condition("5_jepa_rpe_consolidation", "JEPA+Consol", "#7b2d8b"),
    ]
    from analysis.dl2l_analysis.config import REPO_ROOT
    return ExperimentAnalysis(
        name="rotten_fruit_v1",
        conditions=conditions,
        trials=5,
        data_dir=REPO_ROOT / "ml" / "data_rotten_fruit_v1",
    )
...
def run(cfg: ExperimentAnalysis | None = None) -> None:
    ...
    sleep_ep = load_all(cfg, "sleep_episodes.parquet")  # noqa: F841 (parity w/ original; unused there too)
    ...
    plt.tight_layout()
    save(fig, "02_rotten_consumption.png", cfg)
    ...
    rotten_perc_ranked = attach_tick_rank(rotten_perc, creatures)
    ...
    plt.tight_layout()
    save(fig, "03_rotten_perception_response.png", cfg)
    ...
    fig.suptitle(
        ...
        fontsize=12)
    plt.tight_layout()
    save(fig, "04_food_preference.png", cfg)
    ...
    plt.tight_layout()
    save(fig, "06_neuromodulators.png", cfg)
