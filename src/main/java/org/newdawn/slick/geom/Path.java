...
public class Path extends Shape {
	 ...
	 */
	public Path(double sx, double sy) {
		localPoints.add(new double[] {sx,sy});
		cx = sx;
		cy = sy;
		pointsDirty = true;
	}
	 ...
	 */
	public void startHole(double sx, double sy) {
		hole = new ArrayList();
		holes.add(hole);
	}
	 ...
	 */
	public void curveTo(double x, double y, double cx1, double cy1, double cx2, double cy2) {
	 ...
	 */
	public void curveTo(double x, double y, double cx1, double cy1, double cx2, double cy2, int segments) {
		...
		for (int i=1;i<segments+1;i++) {
			...
			if (hole != null) {
				hole.add(new double[] {p.x,p.y});
			} else {
				localPoints.add(new double[] {p.x,p.y});
			}
			cx = p.x;
			cy = p.y;
		}
	...
	protected void createPoints() {
		points = new double[localPoints.size() * 2];
		for (int i=0;i<localPoints.size();i++) {
			double[] p = (double[]) localPoints.get(i);
			points[(i*2)] = p[0];
			points[(i*2)+1] = p[1];
		}
	}
	...
	private ArrayList transform(ArrayList pts, Transform t) {
		double[] in = new double[pts.size()*2];
		double[] out = new double[pts.size()*2];
	
		for (int i=0;i<pts.size();i++) {
			in[i*2] = ((double[]) pts.get(i))[0];
			in[(i*2)+1] = ((double[]) pts.get(i))[1];
		}
		t.transform(in, 0, out, 0, pts.size());
		
		ArrayList outList = new ArrayList();
		for (int i=0;i<pts.size();i++) {
			outList.add(new double[] {out[(i*2)],out[(i*2)+1]});
		}
